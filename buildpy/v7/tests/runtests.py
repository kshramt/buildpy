...
def main(argv):
    ...
    @buildpy.v7.DSL.let
    def _():
        puri = buildpy.v7.DSL.uriparse("a/b;c;d?e=f#gh")
    ...
    @buildpy.v7.DSL.let
    def _():
        s = buildpy.v7.DSL.serialize(None)
        assert s == "n", s
        s = buildpy.v7.DSL.serialize(1)
        assert s == "i1_", s
        s = buildpy.v7.DSL.serialize(2)
        assert s == "i2_", s
        s = buildpy.v7.DSL.serialize(1.234e89)
        assert s == "fi22_0x1.f041b8a7a54f1p+295", s
        s = buildpy.v7.DSL.serialize([1.234e89])
        assert s == "li1_fi22_0x1.f041b8a7a54f1p+295", s
        s = buildpy.v7.DSL.serialize([1.234e89, 32])
        assert s == "li2_fi22_0x1.f041b8a7a54f1p+295i32_", s
        s = buildpy.v7.DSL.serialize({1: 1.234e89, 99: ["b", 4], 3.2: {"p": 9, "r": -9831.98773}})
        ...
        assert buildpy.v7.DSL.serialize(dict(a=1, b=2, c=3)) == buildpy.v7.DSL.serialize(dict(c=3, b=2, a=1)) == buildpy.v7.DSL.serialize(dict(b=2, a=1, c=3))
    ...
    def _():
        ...
        def _():
            tmp0 = os.getcwd()
            tmp1 = tempfile.gettempdir()
            with buildpy.v7.DSL.cd(tmp1) as c:
                tmp2 = os.getcwd()
                comp(c.old, tmp0)
                comp(c.new, tmp2)
            tmp3 = os.getcwd()
