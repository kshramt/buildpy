...
def main(argv):
    result = doctest.testmod(buildpy.v4)
    if result.failed > 0:
        exit(1)
    ...
    def _():
        ...
        assert buildpy.v4._serialize(dict(a=1, b=2, c=3)) == buildpy.v4._serialize(dict(c=3, b=2, a=1)) == buildpy.v4._serialize(dict(b=2, a=1, c=3))
    ...
    def _():
        ...
        def _():
            tmp0 = os.getcwd()
            tmp1 = tempfile.gettempdir()
            with buildpy.v4.DSL.cd(tmp1) as c:
                tmp2 = os.getcwd()
                comp(c.old, tmp0)
                comp(c.new, tmp2)
            tmp3 = os.getcwd()
